...
const probe = async (path) => {
  ...
  try {
    ...
    const platformStatusOk = path !== '/api/v1/platform/status' || (
      response.ok &&
      contentType.toLowerCase().includes('application/json') &&
      body?.ok === true &&
      body?.dynamic === true &&
      body?.status === 'OPERATIONAL' &&
      body?.deployment_authority === 'github-cloudflare' &&
      body?.domain_authority === 'cloudflare' &&
      body?.release_policy === 'fail-closed' &&
      body?.dynamic_contract === 'active'
    );
    const status = response.ok && healthOk && runtimeOk && platformStatusOk ? 'PASS' : 'FAIL';
    return {
      path,
      expected: path === '/healthz' ? '200 + body ok' : path === '/api/v1/platform/status' ? '2xx + valid JSON + operational contract' : path.startsWith('/api/v1/') ? '2xx + dynamic=true' : '2xx',
