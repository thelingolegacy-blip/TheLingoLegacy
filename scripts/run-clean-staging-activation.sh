...
for v in STAGING_VERIFY_AUTH_CMD STAGING_BASELINE_CMD STAGING_MUTATE_CLOUD_RUN_CMD STAGING_MUTATE_FIREBASE_CMD STAGING_MUTATE_WORKER_CMD STAGING_POST_MUTATION_VERIFY_CMD STAGING_PROMOTE_CMD STAGING_POST_PROMOTION_VERIFY_CMD STAGING_ROLLBACK_WORKER_CMD STAGING_ROLLBACK_CLOUD_RUN_CMD STAGING_ROLLBACK_FIREBASE_CMD STAGING_ROLLBACK_VERIFY_CMD; do req "$v"; done
...
echo "STAGING_RUN=CLEAN_ACTIVATION"; echo "STARTED_AT=$(date -u +%Y-%m-%dT%H:%M:%S.%3NZ)"
echo "AUTHORIZATION_VERIFY"; bash -lc "$STAGING_VERIFY_AUTH_CMD"; echo "AUTHORIZED"
