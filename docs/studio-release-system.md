...
5. **Monitoring** — watch deployment health and reported errors after launch; enable paid telemetry only after approval.
...
- Production pages include canonical metadata and social metadata; paid telemetry scripts stay disabled by default.
...
- Review only approved no-cost traffic signals unless paid telemetry is enabled later.
- Use manual QA and approved no-cost checks unless paid performance telemetry is enabled later.
